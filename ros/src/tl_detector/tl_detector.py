...
class TLDetector(object):
    ...
    def image_cb(self, msg):
        ...
        if self.state != state:
            self.state_count = 0
            self.state = state
        # threshold is used to ensure safety
        elif self.state_count >= STATE_COUNT_THRESHOLD:
            # publish the red light waypoint position
            self.last_state = self.state
            light_wp = light_wp if state == TrafficLight.RED else -1
            self.last_wp = light_wp
            self.upcoming_red_light_pub.publish(Int32(light_wp))
        else:
            # Otherwise publish last red waypoint position
    ...
    def process_traffic_lights(self):
        ...
        if(self.pose):
            car_position_idx = self.get_closest_waypoint(self.pose.pose.position.x, \
        ...
        for i, light in enumerate(self.lights):
            ...
            # for each traffic light, there is a corresponding stop line
            line = stop_line_positions[i]
            stop_line_wp_idx = self.get_closest_waypoint(line[0], line[1])
            # Find closest stop line waypoint index
            d = stop_line_wp_idx - car_position_idx
            # d >=0 means stop line is in front of car
            if d >= 0 and d < diff:
                diff = d
                closest_light = light
