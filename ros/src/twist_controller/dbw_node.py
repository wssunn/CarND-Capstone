...
class DBWNode(object):
    ...
    def loop(self):
        # 50Hz here is important as it is below 20 Hz DBW will shut down
        rate = rospy.Rate(50) # 50Hz
        while not rospy.is_shutdown():

            # Get predicted throttle, brake, and steering using `twist_controller`
            # if all of these are initialised, values obtained by subscribe from nodes
            if not None in (self.current_vel, self.linear_vel, self.angular_vel):
                self.throttle, self.brake, self.steering = self.controller.control(self.current_vel,
                                                                                   self.dbw_enabled,
                                                                                   self.linear_vel,
                                                                                   self.angular_vel)
            # only publish the control commands if dbw is enabled                                                                       self.angular_vel)
            if self.dbw_enabled:
